...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if($user_data && password_verify($password, $user_data['password'])) {
        ...
        $_SESSION['full_name'] = $user_data['full_name']; // Corrected: Use 'full_name' from the database
        $_SESSION['role'] = $user_data['role']; // Corrected: Use 'role' from the database
        $_SESSION['email'] = $email;

        // Redirect to the appropriate dashboard based on user role
        header("Location: ../{$_SESSION['role']}/{$_SESSION['role']}_dashboard.php"); // Corrected: Use $_SESSION['role']
        exit();
    } else {
